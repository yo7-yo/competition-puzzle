...
public class nineMode extends JFrame implements MouseListener {
    ...
    private String getImageName(String path) {
        switch (path) {
            case "image/太和殿.png": return "太和殿";
            case "image/乾清宫.png": return "乾清宫";
            default: return "皇宫建筑";
        }
    }
        ...
        switch (path) {
            case "image/太和殿.png":
                return "<html><body style='font-family:微软雅黑;font-size:16px;line-height:1.8;'>"
                    + "<h2 style='color:#8B4513;text-align:center;'>太和殿</h2>"
                    + "<hr style='border:1px solid #D2B48C;'>"
                    + "<p><b>📍 位置：</b>北京故宫核心位置</p>"
                    + "<p><b>📅 年代：</b>明清两代</p>"
                    + "<p><b>🏛️ 规模：</b>面阔十一间，进深五间，三层汉白玉须弥座台基</p>"
                    + "<p><b>🎨 特色：</b>重檐庑殿顶，金黄色琉璃瓦，金龙和玺彩画，金碧辉煌</p>"
                    + "<p><b>🏆 地位：</b>中国古代宫殿建筑的最高杰作，皇帝举行重大典礼的场所</p>"
                    + "</body></html>";
            case "image/乾清宫.png":
                return "<html><body style='font-family:微软雅黑;font-size:16px;line-height:1.8;'>"
                    + "<h2 style='color:#8B4513;text-align:center;'>乾清宫</h2>"
                    + "<hr style='border:1px solid #D2B48C;'>"
                    + "<p><b>📍 位置：</b>北京故宫内廷</p>"
                    + "<p><b>📅 年代：</b>明清两代</p>"
                    + "<p><b>🏛️ 规模：</b>面阔九间，重檐歇山顶，金黄色琉璃瓦</p>"
                    + "<p><b>🎨 特色：</b>汉白玉栏杆，龙凤呈祥栏板雕刻，朱红色圆柱，莲花座柱础</p>"
                    + "<p><b>🏆 地位：</b>明清皇帝寝宫，皇家宫殿建筑典范</p>"
                    + "</body></html>";
            default:
                return "<html><body style='font-family:微软雅黑;font-size:16px;line-height:1.8;'>"
                    + "<h2 style='color:#8B4513;text-align:center;'>皇宫建筑</h2>"
                    + "<hr style='border:1px solid #D2B48C;'>"
                    + "<p>中国古代皇宫建筑，体现了皇家宫殿的恢弘气势与精湛工艺。</p>"
                    + "</body></html>";
        }
    }

    public nineMode(int num, int times){
        this(num, times, null);
    }

    public nineMode(int num, int times, JFrame previousFrame){
        this.num=num;
        this.times=times;
        ...
        addWindowListener(new WindowAdapter() {
            ...
            public void windowClosing(WindowEvent e) {
                int result = JOptionPane.showConfirmDialog(nineMode.this, "确定要退出吗？", "确认退出", JOptionPane.YES_NO_OPTION);
                if (result == JOptionPane.YES_OPTION) {
                    ...
                    // 记录失败结果
                    Check.getRecord("皇宫建筑", num, times, false);
    ...
    public void init() {
        ...
        buttonrule.addActionListener(e->{
            new Introduction(num,times,3,false);
        });
        ...
        while(ci++<10) {
            try {
                // 九宫格模式只能使用皇宫建筑图片
                String[] allowedImages = {"image/太和殿.png", "image/乾清宫.png"};
                ...
                // 使用宫殿模式保存的图片
                String modeImage = file.PictureConfig.getSelectedImageForMode(file.PictureConfig.MODE_PALACE);
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if((llabel.getRow()==label.getRow()+1||llabel.getRow()==label.getRow()-1)&&(llabel.getCol()<=label.getCol()+1&&llabel.getCol()>=label.getCol()-1)||(llabel.getCol()==label.getCol()+1||llabel.getCol()==label.getCol()-1)&&(llabel.getRow()<=label.getRow()+1&&llabel.getRow()>=label.getRow()-1)){
            Icon a;
            ...
            //完成提示+贴图
            if(result()) {
                new WinCue(this,num,times,3,music,previousFrame);
            }
        ...
        countdownTimer = new Timer(1000, e -> {
            ...
            if (result() || remainingSeconds[0] <= 0) {
                ...
                SwingUtilities.invokeLater(() -> {
                    if(!result()) new DefaultCue(this,num,times,3,music,previousFrame);
                });
                return;
            }
            remainingSeconds[0]--;
